 ...
 * 
 * <p>RankThenEncipher encrypts a given plaintext in such a way that the output is in the same format as the input.
 * The format is given by a message space delivered in the constructor. There are no restrictions concerning the type of the message space, all message spaces which implement the MessageSpaces interface are allowed.</p>
 * 
 * <p>Implemented MessageSpaces in this package are:</p>
 * <ul>
 *   <li>StringMessageSpace: Format can be given by a regular expression or a dfa automaton</li>
 ...
 * </ul>
 * <p>For further information about a specific MessageSpace consult its documentation.</p>
 * 
 * <p>The main idea behind the "Rank-then-Encipher Approach" is that the algorithm does not directly encrypt the input value, e.g. a string, but assigns him first a number.<br>
 ...
 * For further details about the "Rank-then-Encipher Approach" visits its documentation on the link given above.</p>
 * 
 * <p>Following a simple example how to use a RankThenEncipher with an EnumerationMessageSpace:</p>
 * 
 * <pre><code>String[] countries = new String[]{"Austria","Canada","France","Germany","Mexico","Poland","Spain","Switzerland","United States"};
 * 
 * EnumerationMessageSpace&lt;String&gt; messageSpace = new EnumerationMessageSpace&lt;String&gt;(Arrays.asList(countries));
 * RankThenEncipher&lt;String&gt; rankThenEnc = new RankThenEncipher&lt;String&gt;(messageSpace);
 *	
 * String encCountry = rankThenEnc.encrypt("Switzerland", key, tweak); //possible encryption result: "Mexico"
 * String decCountry = rankThenEnc.decrypt(encCountry, key, tweak);//decryption brings back "Switzerland"</code></pre>
 * 
 * <p>If you try to encrypt or decrypt a value that is not defined in the message space, a OutsideMessageSpaceException is thrown.<br>
 * <code>String encCountry = rankThenEnc.encrypt("Afghanistan", key, tweak);</code><br>
 * This code would throw an Exception because "Afghanistan" is a country not defined in the EnumerationMessageSpace.</p>
 * 
 * <p>Following an example how to use RankThenEncipher with an StringMessageSpace where a regular expression for valid MasterCard credit card numbers is defined:</p>
 * <pre><code>String masterCardNumbers = "5[1-5][0-9]{14}";
 * StringMessageSpace messageSpace = new StringMessageSpace(masterCardNumbers);
 * 
 * RankThenEncipher&lt;String&gt; rankThenEnc = new RankThenEncipher&lt;String&gt;(messageSpace);
 * String encCardNumber = rankThenEnc.encrypt("5500187004490131", key, tweak); //possible encryption result: "5333831844603012"</code></pre>
 * 
